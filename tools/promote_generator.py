...
💡 **发布建议：**
- 复制上方文案到 LinkedIn 动态发布
- 建议配上 1-3 张产品实拍图或工厂照片
- 在评论区置顶留下联系方式
- 每周发布 1-2 次，保持活跃
"""

    if actual_lang == "Spanish":
        return f"""📢 **LinkedIn Post (Español)**

> **{company or "Our company"}** specializes in **{product}**, serving clients in {markets}.
>
...
💡 **Posting Tips:**
- Copy the text above and paste into a LinkedIn post
- Attach 1-3 product or factory photos
- Pin your contact info in the first comment
- Post 1-2 times per week
"""

    # Default: English
    return f"""📢 **LinkedIn Post (English)**
...
"""
    ...
    if actual_lang == "Spanish":
        return f"""📘 **Facebook Page Post (Español)**
...
"""
    ...
    return f"""📘 **Facebook Page Post (English)**
...
💡 **操作步骤：** 逐条复制到对应平台的公司简介栏，提交后等待审核通过
"""

    if actual_lang == "Spanish":
        return f"""📚 **Directorio Comercial - Textos para enviar (Español)**

Plataformas donde puedes registrar tu empresa gratis:

**1. Kompass (Global)** — https://www.kompass.com
...
**2. Europages (Europa)** — https://www.europages.co.uk
...
💡 **Instrucciones:** Copia y pega en el formulario de cada plataforma, espera la revisión
"""

    # Default English
    return f"""📚 **Business Directory Submission Text (English)**
...
**2. Europages (Europe)** — https://www.europages.co.uk
> We are a trusted supplier of {product}, serving clients in {markets}. {strength or "High quality, competitive price."} MOQ flexible.

**3. ThomasNet (North America)** — https://www.thomasnet.com
> {company or "Our Company"} is a leading supplier of {product} in {markets}. Contact us for a quote.

**4. Made-in-China.com (China export platform)** — Register for free, upload products, get overseas inquiries
...
💡 **How to use:** Copy and paste into each platform's company profile form, then wait for review
"""
...
def _gen_twitter_post(product, company, website, markets, lang) -> str:
    ...
    if actual_lang == "中文":
        return f"""🐦 **Twitter / X 推文文案（中文，280字符内）**
...
💡 **发布建议：** 建议每天发 1 条，配合产品图片， hashtag 控制在 3-5 个
"""

    if actual_lang == "Spanish":
        return f"""🐦 **Twitter / X Tweets (Español, under 280 chars)**
...
💡 **Posting Tips:** Post 1 per day with product photos, 3-5 hashtags per tweet
"""

    return f"""🐦 **Twitter / X Tweets (English, under 280 chars)**
